...
public abstract class BaseQuickAdapter<T, H extends BaseAdapterHelper> extends BaseAdapter {
    ...
    /**
     * Same as QuickAdapter#QuickAdapter(Context,int) but with some initialization object.
     * 
     * @param context The context.
     * @param layoutResId The layout resource id of each item.
    ...
    public int getItemViewType(int position) {
        if (displayIndeterminateProgress) {
            if (mMultiItemSupport != null)
                return position >= data.size() ? 0 : mMultiItemSupport.getItemViewType(position,
                        data.get(position));
        } else {
            if (mMultiItemSupport != null)
     ...
     * 
     * @param position The position of the item within the adapter's object set of the item whose view we want.
     * @param convertView The old view to reuse, if possible. Note: You should check that this view is non-null and of an appropriate type
     *            before using. If it is not possible to convert this view to display the correct object, this method can create a new view.
     *            Heterogeneous lists can specify their number of view types, so that this View is always of the right type (see
